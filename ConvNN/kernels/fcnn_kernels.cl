...
__kernel void compout(  global Node*  nodes,global Node * prevnodes,int softflag)
{
...
__kernel void softmax( global Node*  nodes,int nodesnum){
...
__kernel void backprophid(global Node*  nodes,global Node * prevnodes,global Node *nextnodes,int nextnumNodes,float a)
{
    const int n = get_global_size(0);
    const int i = get_global_id(0);
...
__kernel void backpropout(global Node*  nodes,global Node * prevnodes,global float* targets,float a,int softflag )
{
	const int n = get_global_size(0);
	const int i = get_global_id(0);
